...
class Mock {
    ...
    fun getPhrase(category: Int): Phrase? {
        val newList = mListPhrase.filter { it.CategoryId == category }

        if (newList.isNotEmpty()) {
            val randomIndex = Random.nextInt(newList.size)
            return newList[randomIndex]
        }
        val randomIndex = Random.nextInt(mListPhrase.size)
        return mListPhrase[randomIndex]
    }
}